<script lang="ts">
	...
	function startAnimation() {
		if (svgRef && !isAnimating) {
			...
			if (circle && rect) {
				...
				// Phase 2: Slide circle from left to right (OFF to ON)
				setTimeout(() => {
	...
	function animateTrackHighlight(rect: Element) {
		// Track glows to indicate activation
	...
	function animateCircleSlide(circle: Element) {
		// Circle slides from left (cx=9) to right (cx=15)
...
<div 
  ...
  aria-label="toggle-left-icon icon"
  {...restProps}
>
  <svg
    bind:this={svgRef}
