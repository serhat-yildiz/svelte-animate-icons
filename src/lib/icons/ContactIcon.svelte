<script lang="ts">
	...
	function startAnimation() {
		if (svgRef) {
			...
			if (circle) {
				...
				], { duration: 600, delay: 250, easing: 'ease-out' });
			}
			
			// Lines animation (delay: 0.4)
			const lines = svgRef.querySelectorAll('path[d*="v2"]');
			lines.forEach(line => {
				line.animate([
					{ transform: 'translateX(-10px)', opacity: '0' },
					{ transform: 'translateX(0px)', opacity: '1' }
				], { duration: 400, delay: 400, easing: 'ease-out' });
			});
	...
	export function getControls(): ContactIconHandle {
		isControlled = true;
...
>
	...
	>
		<path d="M16 2v2" />
		<path d="M7 22v-2a2 2 0 0 1 2-2h6a2 2 0 0 1 2 2v2" stroke-dasharray="30" stroke-dashoffset="30" />
		<path d="M8 2v2" />
