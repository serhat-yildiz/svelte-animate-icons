<script lang="ts">
	...
	function startAnimation() {
		if (svgRef) {
			...
			// Arrow head animation (delay: 0.05)
			...
			// Tray drawing (delay: 0.1)
			const tray = svgRef.querySelector('path[d*="M21 15"]');
			if (tray) {
				tray.animate([
					{ strokeDashoffset: '60', opacity: '0.3' },
					{ strokeDashoffset: '0', opacity: '1' }
				], { duration: 600, delay: 100, easing: 'ease-in-out', fill: 'forwards' });
			}
		}
	}
	...
	export function getControls(): DownloadIconHandle {
		isControlled = true;
