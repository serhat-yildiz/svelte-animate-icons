<script lang="ts">
	  ...
	  if (paths[1]) {
		currentAnimations.push(
		  paths[1].animate(
			...
			{ duration, iterations: loop || autoPlay || currentState === 'loading' ? Infinity : 1, easing: 'ease-in-out' }
		  )
		);
	  }
	  if (paths[0]) {
		currentAnimations.push(
		  paths[0].animate(
			...
			{ duration, delay: 200, iterations: loop || autoPlay || currentState === 'loading' ? Infinity : 1, easing: 'ease-in-out' }
		  )
		);
	  }
  ...
  >
	...
	>
	  <path d="m6 9 6 6 6-6" style="opacity: 0.5;" />
	  <path d="m6 9 6 6 6-6" />
	</svg>
  </div>
